...
function drawBlock(px, py, color, ctx = ctx, size = SIZE) {
  ctx.fillStyle = color;
...
function drawNext() {
  const nextSizes = [40, 30, 25, 20, 15]; // 各ネクストピースのブロックサイズを定義
  nextCanvases.forEach((ctxN, i) => {
    ...
    const size = nextSizes[i]; // 定義したサイズを使用
    const xOffset = (ctxN.canvas.width - shape[0].length * size) / 2;
    const yOffset = (ctxN.canvas.height - shape.length * size) / 2;
    shape.forEach((r, y) => r.forEach((v, x) => {
      if (v) {
        ctxN.fillStyle = colors[type];
        ctxN.fillRect(x * size + xOffset, y * size + yOffset, size, size);
        ctxN.strokeStyle = "#111";
        ctxN.strokeRect(x * size + xOffset, y * size + yOffset, size, size);
      }
    }));
  });
}
...
function drawHold() {
  ...
  const size = 30; // 固定サイズを使用
  const xOffset = (120 - shape[0].length * size) / 2;
  const yOffset = (120 - shape.length * size) / 2;
  shape.forEach((r, y) => r.forEach((v, x) => {
    if (v) {
      holdCtx.fillStyle = colors[holdType];
      holdCtx.fillRect(x * size + xOffset, y * size + yOffset, size, size);
      holdCtx.strokeStyle = "#111";
      holdCtx.strokeRect(x * size + xOffset, y * size + yOffset, size, size);
    }
  }));
}
